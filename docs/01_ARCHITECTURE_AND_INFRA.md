...
2. **Stack Logistique (Docker Compose)** :
    - **OSRM (Backend)** : Moteur de routage (Données Brussels Capital Region).
...
- Préparer un script `setup-osrm.sh` pour automatiser l'extraction des données OpenStreetMap Brussels (`osrm-extract`, `osrm-partition`, `osrm-customize`).
